...
You can try dismissing some other items as well, but observe something. I'm going to select Flip Flops, and then swipe towards the right and that also dismisses the item. Well, that may not be exactly what you're looking for. It turns out that the Dismissible widget allows you to swipe left or right to dismiss a particular item. What you really want is to specify the DismissDirection. I have set the DismissDirection.endToStart. Meaning by default, if you swipe from the right to the left, your widget will be dismissed, and this will give us the behavior that we are looking for.

I'm going to restart my app so I have all my list elements. I'm going to try and swipe from the left to the right. That's not going to dismiss the list items. But, when I swipe from the right to the left, well, Clogs is going to get dismissed because the swipe was in the right DismissDirection.endToStart. You can try this with some other elements as well. Right to left will work. There are lots of little nice features available with the Dismissible widget.

Let me show you one. The background property allows you to specify what background you want displayed when a particular widget is being dismissed. Here I've set the background to a Container that is red in color. The color property sets this up. I've also set a child of this Container to the delete Icon that will be displayed in the white color. The AlignmentDirectional.centerEnd  will ensure that the delete Icon will be displayed at the very right side of the Container. 

I'm going to drag very slowly towards the left. Notice the red color and the delete icon. It's definitely kind of a cool effect to have with your list view. Let's try this with Open-toed shoes, once again, you see the red background and the delete icon. One last little detail that you might want to add. Each time you delete an item, it makes sense to show a little alert or a indicator that a particular item has been dismissed or deleted. I'll do this using a SnackBar.

Within the onDismissed handler, I've added a little bit of code to display a SnackBar indicating that a particular product has been deleted. You simply have the Text, the product name, and deleted.

## Multiple Gesture Detectors
Now, within your mobile app, it's quite possible that you have two widgets that overlap one another and both of them have GestureDetectors that detect the same gesture. Let's say it's the onTap. So, when you tap on a widget, which GestureDetector wins? Which callback is invoked? That's exactly what we are going to be studying in this demo.

I have two squares, the outer orange square and the inner blue square. You'll see this in the code in just a bit. These two square containers are essentially placed one on top of another using a stack. So, both of these containers are sibling widgets that are part of the stack. Let's turn our attention to the code and let's see how multiple gesture detectors behave.

You can see that the children property of the stack has two GestureDetectors as children. These are the two GestureDetectors, both of which respond to the onTap handler. So, both of these GestureDetectors are looking for the same gesture, the onTap. Observe that these GestureDetectors are sibling widgets that is they're at the same level.

So, even though when the widgets are displayed, one square is layered on top of another, they're actually sibling widgets in the widget tree. Let's expand and take a look at the first of these widgets that is the outer orange square. 

I have the onTap handler defined for this GestureDetector. You see that I simply print out to the Run console Outer onTap, and I set the _winner to be the outer GestureDetector. I also have debugPrint statements for tap down and tap cancel. You can choose to leave them in there.  It's easy to figure out what's going on in code. This GestureDetector wraps a Container. If the _winner is equal to the outer container, that is the orange container, then the _winner will have a highlighted border, a red border around it. Otherwise, there'll be no highlight, no border.

You can play around with this app, and you'll find that each time you click on the outer box, that will be highlighted. Only one gesture recognizer will enter the arena, the Outer onTap wins. When you click on the inner container, the Inner onTap will win. Again, only one GestureDetector will enter the arena. Even though the containers are visually one on top of the other, the gestures are clearly disambiguated. Flutter is clearly able to tell which container we actually clicked on.

## Nested Gesture Detectors
you'll find that the look and feel of the resulting app is exactly the same. So, I'm going to rerun the app, and you'll see that I have an outer orange container and an inner blue container along with the Reset button. Now you might ask me if the app looks exactly the same, I have an outer container and an inner container, then what's different? The difference is in how I have structured the two containers. I have the inner container as a child of the outer container.

The inner and outer container are set up in such a way so that they are no longer siblings, but the inner green container is a child of the outer orange container. Also, both of these containers are wrapped in GestureDetectors, which means we have nested gesture detectors. The GestureDetector of the blue box is nested within the GestureDetector of the orange box. 

I'm going to click on the orange box. You can see the orange box is now highlighted, and if you look at the Console output, you can see a new gesture arena has been opened up and if you scroll down a bit, you'll see that exactly one recognizer entered this arena, and that was the Default winner. So, when we clicked on the outer orange parent container, only the GestureDetector for that container entered the arena and it won. And you can see Outer onTapDown and Outer onTap is printed to screen.

Now I'm going to go ahead and click on the green container, the inner container. You can see that's the container that is highlighted using the red outline. Here is where the Console messages get interesting, a new gesture arena, Gesture arena 8 was opened up. Observe that two gesture recognizers were added to the arena. There are two Adding statements, one for the child gesture recognizer and one for the parent because the tap was propagated from the child to the parent. This is further confirmed because the tap down event was received by the Inner as well as the Outer container.

Inner onTapDown is printed first, indicating that received the event first, and then Outer TapDown is printed. There are two gesture recognizers in the gesture arena, but only one of them is the Winner. Notice that the Winner is the TapGestureRecognizer#29e67, and that happens to be the inner green container's gesture recognizer. You can also tell from the Console messages, Inner onTap is executed, that's printed out to screen, but the Outer gesture recognizer is canceled. You can see the Outer gesture recognizer prints out onTapCancel. Each time you click on the inner widget, two gesture recognizers enter the arena, the parent as well as the child. It is the child gesture recognizer that is the winner in this arena. In the Console window, you can see there are two Adding messages for the two gesture recognizers that entered.

The tap down is invoked for both the Inner as well as the Outer box, you'll find that the tap handler is invoked only for the inner box. So, you see Inner onTap. The Outer tap is canceled. You can see the onTapCancel there. So, when you have multiple gesture detectors, one nested within another, you'll find that all of them receive the tap but the gesture recognizer that wins in the gesture arena is the innermost child gesture recognizer on which you actually performed the tap.

## Disambiguating Gestures Using Raw Gesture Detectors
we'll see, how I've already set up the code for our app here, and I've run the code so you can see what this app looks like. If you look at the emulator, you'll see there are two buttons, one nested within another. The outer orange button says Like us, and the inner green button says Donate. 

The GestureDetectors have a parent-child relationship, which means if you click on the Donate button, that'll be the gesture recognizer that wins in the gesture arena. The Like us will not win if you click on Donate, and that's the behavior that I'm going to change. First, let's take a look at our app with the default behavior. I've set the debugPrintGestureArenaDiagnostics to true so you can see what exactly is going on within the gesture arena.

It's pretty clear that the onTap handlers for the Donate as well as the Like us button were both activated. You can also confirm this by looking at the output of the Run Console. Both the messages donate onTap and likeUs onTap are printed out to screen. Let's scroll up and take a look at what happened in the Gesture arena 15. When we tapped on the Donate button because it's a child of the Like us button, two gesture recognizers entered the arena. That's why you see two Adding messages right here.
